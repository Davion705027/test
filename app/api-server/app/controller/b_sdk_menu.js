...
class BSDKMenuController extends Controller {
  /**
   * 查询  全部数据
   * @returns
   */
  async index() {
    ...
    try {
      ...
      const result = await ctx.model.BSdkMenu.paginate(query, options);
  ...
  async show() {
    ...
    try {
      ...
      const result = await ctx.model.BSdkMenu.findById(id).exec();
      ctx.api_success_data(result);
    } catch (error) {
      ctx.api_error_msg(error);
    }
  ...
  compute_final_obj_when_create_or_update() {
    const ctx = this.ctx;
    let { id, names, fatherId, mark, status, type, order, related_doc } =
      ctx.request.body;
    let final_obj = {
      id,
      names,
  ...
  async create_or_update_inner_function_update_father(final_obj) {
    console.log("final_obj--", final_obj);
    const ctx = this.ctx;
    const resultfather = await ctx.model.BSdkMenu.findById(final_obj.fatherId).exec();
  ...
  async create() {
    ...
    try {
      ...
      const result = await ctx.model.BSdkMenu.create(final_obj);
  ...
  async update() {
    ...
    try {
      //旧的父级ID
      // let old_father_obj = await  ctx.model.BSdkMenu.findById(id, final_obj)
      let final_obj = this.compute_final_obj_when_create_or_update();
      let id = final_obj.id;
      let options = {
        returnDocument: "after",
      };
      const result = await ctx.model.BSdkMenu.findByIdAndUpdate(id, final_obj);
  ...
  async destroy() {
    ...
    try {
      ...
      const resultdoc = await ctx.model.BSdkMenu.findById(id).exec();
      if (resultdoc.fatherId == -1) {
        ctx.api_error_msg("根节点不能更改");
      } else {
        const result = await ctx.model.BSdkMenu.findByIdAndDelete(id);
        ctx.api_success_data(result);
      }
    } catch (error) {
      ctx.api_error_msg(error);
  ...
  async disable() {
    ...
    try {
      ...
      if (resultdoc.fatherId == -1) {
        ...
        const result = await ctx.model.BSdkMenu.findByIdAndUpdate(id, update);
        ctx.api_success_data(result);
      }
    } catch (error) {
      ctx.api_error_msg(error);
  ...
  async findAllMenus() {
    ...
    const result = await this.ctx.model.BSdkMenu.find(query, "", {
      sort: { order: 1, updatedAt: -1, },
    }).exec();
  ...
  async find_children_deep() {
    ...
    try {
      ...
      let result = await ctx.model.BSdkMenu.find(query, "", options).exec();
...
module.exports = BSDKMenuController;
