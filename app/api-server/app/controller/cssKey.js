 ...
 * @FilePath: /client-api-doc-server/app/controller/cssKey.js
 * @Description    :
 */
"use strict";
const Controller = require("egg").Controller;
class CssKeyController extends Controller {
  /**
   * 查询  全部数据
   * @returns
   */
  async index() {
    ...
    try {
      ...
      const result = await ctx.model.CssKey.paginate(query, {
  ...
  async show() {
    ...
    try {
      ...
      const result = await ctx.model.CssKey.findById(id).exec();
      ctx.api_success_data(result);
    } catch (error) {
      ctx.api_error_msg(error);
    }
  ...
  async create() {
    ...
    try {
      ...
      const result = await ctx.model.CssKey.create(final_obj);
      ctx.api_success_data(result);
    } catch (error) {
      ctx.api_error_msg(error);
    }
  ...
  async update() {
    ...
    try {
      ...
      let id = final_obj.id;
      delete final_obj.id;
      const old_obj = await ctx.model.CssKey.findByIdAndUpdate(id, final_obj);
      const new_obj = await ctx.model.CssKey.findById(id).exec();
      await this.service.keyChangeRecord.key_change({
        old_obj:  old_obj.toJSON(), 
        new_obj:new_obj.toJSON(), 
        key_type:ctx.assistant.key_type.css
      });
  ...
  async destroy() {
    ...
    try {
      ...
      const result = await ctx.model.CssKey.findByIdAndDelete(id);
      ctx.api_success_data(result);
    } catch (error) {
      ctx.api_error_msg(error);
    }
...
module.exports = CssKeyController;
