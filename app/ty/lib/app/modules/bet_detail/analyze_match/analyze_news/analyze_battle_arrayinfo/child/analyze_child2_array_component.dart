...
class AnalyzeChild2ArrayInfoComponent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<AnalyzeBattleArrayLogic>(
        id: "analyzeChildArrayComponent2",
        builder: (controller) {
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        AnalyzeDivider(),
                        ItemHeaderWidget(
                          name:   '${LocaleKeys.analysis_football_matches_starting_lineup.tr}${controller.state.analyzeArrayPersonEntity2.value.homeFormation ?? ""}',
                        ),
                        AnalyzeDivider(),
                        buildBody(),
                      ],
    ...
    return GetBuilder<AnalyzeBattleArrayLogic>(
        ...
        builder: (controller){
          return Column(
            children: [
              AnalyzeDivider(),
              ( controller.state.analyzeArrayPersonEntity2.value.down==null||controller.state.analyzeArrayPersonEntity2.value.down?.isEmpty ==
                  true)
                ...
                child: ListView.separated(
                  ...
                  scrollDirection: Axis.vertical,
                  itemCount: controller.state.analyzeArrayPersonEntity2.value.down?.length??0,
                  shrinkWrap: true,
                  physics: NeverScrollableScrollPhysics(),

                  //列表项构造器
                  itemBuilder: (BuildContext context, int index) {
                    return  AnalyzeChildItem(controller.state.analyzeArrayPersonEntity2.value.down?[index]??Up());
                  },
                  //分割器构造器
  ...
  buildList() {
    return GetBuilder<AnalyzeBattleArrayLogic>(
        ...
        builder: (controller){
          return
            ( controller.state.analyzeArrayPersonEntity2.value.up==null||controller.state.analyzeArrayPersonEntity2.value.up?.isEmpty ==
              true)
              ?TextNoData(): Container(
            /// 最小高度单个item的高度
            constraints: BoxConstraints(maxHeight: 520.h, minHeight: 100.h),
            ...
            child: ListView.separated(
              padding: EdgeInsets.symmetric(horizontal: 14.w),
              scrollDirection: Axis.vertical,
              itemCount: controller.state.analyzeArrayPersonEntity2.value.up?.length??0,
              shrinkWrap: true,
              physics: NeverScrollableScrollPhysics(),

              //列表项构造器
              itemBuilder: (BuildContext context, int index) {
                return   AnalyzeChildItem(controller.state.analyzeArrayPersonEntity2.value.up?[index]??Up());
              },
              //分割器构造器
              separatorBuilder: (BuildContext context, int index) {
                return AnalyzeDivider(
