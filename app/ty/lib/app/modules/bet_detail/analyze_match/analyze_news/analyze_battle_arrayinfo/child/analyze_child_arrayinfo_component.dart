...
class AnalyzeChildArrayInfoComponent extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return GetBuilder<AnalyzeBattleArrayInfoLogic>(
        id: "analyzeChildArrayInfoComponent",
        builder: (controller) {
        ...
        builder: (controller) {
          return Column(
            children: [
              ...
              AnalyzeDivider(),
              ( controller.state.analyzeArrayPersonEntity.value.down==null||controller.state.analyzeArrayPersonEntity.value.down?.isEmpty ==
                  true)
                  ? TextNoData()
                  ...
                  : Container(
                      ...
                      child: ListView.separated(
                        ...
                        scrollDirection: Axis.vertical,
                        itemCount: controller.state.analyzeArrayPersonEntity
                                .value.down?.length ??
                            0,
                        shrinkWrap: true,
  ...
  buildList() {
    return GetBuilder<AnalyzeBattleArrayInfoLogic>(
        id: "buildList",
        builder: (controller) {
          print("数据空${controller.state.analyzeArrayPersonEntity.value.up}");
          return  (controller.state.analyzeArrayPersonEntity.value.up==null||controller.state.analyzeArrayPersonEntity.value.up?.isEmpty==true)
              ? TextNoData()
              : Container(
                  /// 最小高度单个item的高度
                  constraints:
                  ...
                  child: ListView.separated(
                    ...
                    itemCount: controller
                            .state.analyzeArrayPersonEntity.value.up?.length ??
                        0,
                    shrinkWrap: true,
                    physics: NeverScrollableScrollPhysics(),
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return AnalyzeChildItem(controller.state
                              .analyzeArrayPersonEntity.value.up?[index] ??
                          Up());
                    },
                    //分割器构造器
                    separatorBuilder: (BuildContext context, int index) {
