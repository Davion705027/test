...
class _ScoreListState extends State<ScoreList2> {
  ...
  @override
  void didUpdateWidget(covariant ScoreList2 oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      mainAxisAlignment:
          [3].contains(csid) ? MainAxisAlignment.end : MainAxisAlignment.start,
      children: [
        Row(
          children: [
            // 总分、回合数、总局数
  ...
  /// @description 总局数
  String totalGames() {
    int length = mscConverted.length ?? 0;
    if (length < 1) return '';
    int homeGame = 0;
    int awayGame = 0;
