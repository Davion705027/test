...
/// 详情页显示足球赛事第几节以及赛事时间
class StageChild1 extends StatefulWidget {
  const StageChild1(
      {super.key,
      required this.match,
  ...
  @override
  State<StageChild1> createState() => _StageChild1State();
}

class _StageChild1State extends State<StageChild1> {
  /// 计时器显示时间
  ...
  @override
  void didUpdateWidget(covariant StageChild1 oldWidget) {
  ...
  Widget build(BuildContext context) {
    if (matchResultState()) {
      ...
      // return Countingdown(widget.match);
      ...
      return Row(
        ...
        children: [
          ...
          if (![31].contains(widget.match.mmp))
            if (showTimeCounting())
              AutoSizeText(
          ...
          // 补时
          if (["6", "7"].contains(widget.match.mmp) &&
              ObjectUtil.isNotEmpty(widget.match.mststi) &&
              widget.match.mststi != '0')
            AutoSizeText(
              "+${widget.match.mststi}'",
              maxLines: 1,
              minFontSize: 8,
              style: TextStyle(
                color: widget.isMatchSelect
                    ? Get.theme.subSelectTitleColor
                    : const Color.fromRGBO(204, 221, 255,1),
                fontSize: widget.isPinnedAppbar ? 14.sp : 12.sp,
                fontFamily: 'PingFang SC',
                fontWeight: FontWeight.w500,
              ),
