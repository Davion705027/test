...
class GoingSportWidget extends StatefulWidget {
  ...
  @override
  State<GoingSportWidget> createState() => _GoingSportWidgetState();
}

class _GoingSportWidgetState extends State<GoingSportWidget> {
...
extension _BallsUI on _GoingSportWidgetState {
...
extension _DogHorseUI on _GoingSportWidgetState {
...
extension _TeamsScoreHelper on _GoingSportWidgetState {
...
extension _GoingSportWidgetStateLogic on _GoingSportWidgetState {
  ...
  void onBasketballVideoChanged(
      Duration position,
      bool isFinished,
      Map<String, VrSportReplayDetailScoreRanking?> scoreInfoMap,
      ) {
    if (position.inSeconds < _videoPosition) return;
  ...
  Future<void> _getReplayData() async {
    debugPrint('_replayEntity: ${_replayEntity?.toJson()}');
    final judgementNull = widget.type < 2;
    if (judgementNull) {
      if (_replayEntity != null) return;
